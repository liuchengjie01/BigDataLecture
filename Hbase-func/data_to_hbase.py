...
def list_to_dict(line):
    ...
    if line[1] != '':
        data['cf1:bid'] = line[1]
    if line[2] != '':
        data['cf1:user_id'] = line[2]
    if line[3] != '':
        data['cf1:user_name'] = line[3]
    if line[4] != '':
        data['cf1:content'] = line[4]
    if line[5] != '':
        data['cf1:article_url'] = line[5]
    if line[6] != '':
        data['cf1:src_place'] = line[6]
    if line[7] != '':
        data['cf1:aite_user'] = line[7]
    if line[8] != '':
        data['cf1:topic'] = line[8]
    if line[9] != '':
        data['cf1:forward_num'] = line[9]
    if line[10] != '':
        data['cf1:comment_num'] = line[10]
    if line[11] != '':
        data['cf1:like_nums'] = line[11]
    if line[12] != '':
        data['cf1:publish_time'] = line[12]
    if line[13] != '':
        data['cf1:publish_tool'] = line[13]
    if line[14] != '':
        data['cf1:pic_urls'] = line[14]
    if line[15] != '':
        data['cf1:video_urls'] = line[15]
    if line[16] != '':
        data['cf1:retweet_id'] = line[16]
